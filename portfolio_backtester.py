...
def generate_trade_chart(ticker, df, trades_log, sma_col, report_dir, ma_period, ts_pct, pt_pct):
    """Uses Matplotlib to create and save a detailed trade line chart."""
    ...
    # Plot trade markers and lines
    for trade in trades_log:
        try:
            # Plot Buy Marker
            ax.plot(trade['entry_date'], trade['entry_price'], '^', color='g', markersize=10, label='Buy' if 'Buy' not in [l.get_label() for l in ax.lines] else "")
            
            # Plot Sell Marker
            ax.plot(trade['exit_date'], trade['exit_price'], 'v', color='r', markersize=10, label='Sell' if 'Sell' not in [l.get_label() for l in ax.lines] else "")

            # Plot Profit Target Line (if applicable)
            if trade.get('pt_pct') is not None:
                pt_price = trade['entry_price'] * (1 + trade['pt_pct'] / 100)
                ax.hlines(y=pt_price, xmin=trade['entry_date'], xmax=trade['exit_date'], color='g', linestyle='--', linewidth=1, label='Profit Target' if 'Profit Target' not in [l.get_label() for l in ax.lines] else "")
            
            # Plot Stop Loss Line
            if trade.get('reason') == 'Trailing Stop':
                sl_price = trade['exit_price']
                ax.hlines(y=sl_price, xmin=trade['entry_date'], xmax=trade['exit_date'], color='r', linestyle='--', linewidth=1, label='Stop Loss' if 'Stop Loss' not in [l.get_label() for l in ax.lines] else "")

        except Exception as e:
            print(f"    - Warning: Could not plot trade due to error: {e}")
            continue
    
    pt_str = 'None' if pt_pct is None else str(pt_pct)
    title = f"{ticker} Trades (MA:{ma_period}, TS:{ts_pct}%, PT:{pt_str}%)"
    filename = f"{ticker}_{ma_period}_TS{ts_pct}_PT{pt_str}.png"
